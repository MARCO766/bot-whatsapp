...
async function enviarEventoMeta(usuarioId, nombreEvento, telefono, opciones = {}) {
  try {
    ...
    if (!conexion?.pixel_id || !conexion?.capi_token) {
      console.log("PIXEL/CAPI no configurado");
      return;
    }
...
console.log("PIXEL:", conexion.pixel_id);
console.log("TOKEN:", conexion.capi_token);
console.log("ENVIANDO EVENTO META...");
  ...
  custom_data: {
    ...
    console.log("✅ Evento Meta enviado:", nombreEvento);

  } catch (error) {
    console.log("ERROR ENVIANDO EVENTO META:", error.response?.data || error.message);
  }
}
module.exports = {
  enviarEventoMeta
